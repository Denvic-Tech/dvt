...
Revision ID: 0024
Revises: 0023
Create Date: 2026-03-02 12:29:33.637854
...
# Старый enum (из миграции 0021) - ВЕРХНИЙ регистр (так в базе)
OLD_ENUM = postgresql.ENUM(
    "MYSQL", "POSTGRES", "CLICKHOUSE", "MONGODB", "MSSQL", "KAFKA", "S3", "ORACLE", "CUSTOM",
    name="connectiontype",
)

# Новый enum - нижний регистр + добавлен oracle
NEW_ENUM = postgresql.ENUM(
    "mysql", "postgres", "clickhouse", "mongodb", "mssql", "kafka", "s3", "oracle", 'ftp', 'sftp', "custom",
...
def downgrade() -> None:
    ...
    result = bind.execute(sa.text(
        "SELECT COUNT(*) FROM db_connections WHERE type IN ('ftp', 'sftp')"
    )).scalar()

    if result > 0:
        raise ValueError(
            f"Cannot downgrade: {result} connection(s) with type 'ftp' or 'sftp' exist. "
