...
from core.db.write_v4.errors import WriteV4PlanningError
from core.db.write_v4.models import WritePlan, WriteRequest
...
def plan_write(engine: sa.Engine, request: WriteRequest) -> WritePlan:
    ...
    if not table_exists:
        ...
        raise WriteV4PlanningError(
            f"Target table '{full_name}' does not exist. write_v4 writes only to existing tables."
        )
