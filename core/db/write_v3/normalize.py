...
from core.db.write_v3.models import MissingColumnsMode, WriteDiagnostic, WriteRequest
...
def _is_mssql_uuid_column(column: sa.Column) -> bool:
    normalized = _unwrap_type_wrappers(str(column.type)).lower().replace(" ", "")
    return normalized.startswith("uniqueidentifier")


def _is_mssql_binary_column(column: sa.Column) -> bool:
    normalized = _unwrap_type_wrappers(str(column.type)).lower().replace(" ", "")
    return normalized.startswith("binary") or normalized.startswith("varbinary")


def _coerce_mssql_column_value(table: sa.Table, column: sa.Column, value: object) -> object:
    if value is None or pd.isna(value):
        return None

    if _is_mssql_uuid_column(column):
        ...
        if isinstance(value, str):
            try:
                ...
                raise WriteV3ExecutionError(
                    f"Failed to coerce value {value!r} for MSSQL uniqueidentifier column "
                    f"'{column.name}' in table '{table.fullname}'."
                ) from exc
        raise WriteV3ExecutionError(
            f"Unsupported value {value!r} for MSSQL uniqueidentifier column "
            f"'{column.name}' in table '{table.fullname}'."
        )

    if _is_mssql_binary_column(column):
        ...
        if isinstance(value, str):
            try:
                ...
                raise WriteV3ExecutionError(
                    f"Failed to coerce value {value!r} for MSSQL binary column "
                    f"'{column.name}' in table '{table.fullname}'. Expected hex string."
                ) from exc
        raise WriteV3ExecutionError(
            f"Unsupported value {value!r} for MSSQL binary column "
            f"'{column.name}' in table '{table.fullname}'."
        )
...
def _coerce_mssql_special_columns(normalized: pd.DataFrame, table: sa.Table) -> pd.DataFrame:
    mssql_columns = {
        column.name: column
        for column in table.columns
        if column.name in normalized.columns and (_is_mssql_uuid_column(column) or _is_mssql_binary_column(column))
    }
    if not mssql_columns or normalized.empty:
        return normalized

    coerced = normalized.copy()
    for column_name, column in mssql_columns.items():
        coerced[column_name] = coerced[column_name].map(
            lambda value, *, _table=table, _column=column: _coerce_mssql_column_value(_table, _column, value)
        )
    return coerced
...
def _build_runtime_rename_map(
    ...
    declared_rename_map = getattr(table, "rename_map", None) or {}
    for source_name, target_name in declared_rename_map.items():
        source_name = str(source_name)
        target_name = str(target_name)
        if source_name not in dataframe_names or target_name not in table_column_names:
            continue
    ...
    raise WriteV3ExecutionError(
        ...
        f"{duplicate_pairs}. Rename the DataFrame columns to keep them unique."
    )
...
def align_partition_to_table(
    ...
    *,
    ...
    rename_map = _build_runtime_rename_map(working.columns, table)
    if rename_map:
    ...
    normalized = _coerce_mssql_special_columns(normalized, table)
