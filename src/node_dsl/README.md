...
#### --------- Атрибуты метаданных ---------
- `ICON_KEY: ClassVar[str | None] = None` — стабильный ключ SVG-иконки в UI, например
  `table-from-db`. Передаётся как необязательное `NodeDefinition.icon_key`, не локализуется.
  Несколько нод могут использовать один ключ. Отсутствующий или неизвестный UI ключ
  отображается стандартным цветным маркером.
- SVG хранятся в UI и автоматически собираются в отдельный кешируемый спрайт.
  Backend не читает файлы иконок и не возвращает SVG или URL в определениях нод.
  Для новой иконки добавьте SVG в `src/shared/assets/node-icons` UI-репозитория,
  затем задайте соответствующий `ICON_KEY` в классе ноды.
- `EMOJI` и поле `emoji` сохранены для совместимости со старыми клиентами;
  новый UI использует `icon_key`. Extensions могут выбирать известные UI ключи,
  собственная доставка SVG расширений пока не предусмотрена.
...
## Описания входных полей для UI и агента

`InputField(description=..., agent_description=...)` разделяет две задачи:

- `description` — краткая пользовательская подсказка: назначение параметра, эффект значения
  и поведение при пропуске. Локализуется обычным механизмом `locales/*/nodes.json`.
- `agent_description` — техническая рекомендация по настройке: предварительные проверки,
  критерии выбора, зависимости и типичные ошибки. Хранится рядом с полем на английском,
  не локализуется и не меняет тип, обязательность, default или runtime-валидацию.

Новый аргумент только именованный: существующие позиционные аргументы `InputField` совместимы.
Значение по умолчанию — `None`; старые ноды и extensions не обязаны заполнять его для загрузки.
Оба описания входят в `InputDefinitionModel` и возвращаются через `get_node_definition`.
Наследование нод и `NodeFieldsMixin` сохраняет рекомендации. Поиск нод остаётся компактным.

Для сложного входа с Pydantic-схемой можно описывать вложенные свойства:
`Field(description="...", json_schema_extra={"agent_description": "..."})`.
Эти рекомендации остаются внутри `input_definitions.<input>.schema`; агент должен читать
и внешнее описание, и вложенную схему.

Каждое входное поле built-in ноды должно иметь содержательное `agent_description`, включая
скрытые поля, виджеты, тестовые, экспериментальные и устаревшие ноды. Общие поля описывайте
в базовом классе или mixin; переопределённое поле должно сохранять или уточнять рекомендацию.
Покрытие всех packages и наследуемых полей проверяет unit-тест каталога. Для extensions
поле остаётся необязательным ради совместимости.

Не копируйте пользовательский текст ради заполнения. Для простого параметра достаточно
кратко пояснить формат и критерий выбора; для неоднозначного — необходимые проверки,
зависимости и ограничения фактической реализации. Устаревшие и тестовые ноды должны явно
обозначать своё назначение. Большие сценарии и взаимодействия нескольких параметров
описывайте в README ноды на обоих языках.
Общие MCP instructions и модели патчей описывают протокол и работу с графом; правила конкретных
нод размещайте в их полях и README, а не в глобальном промпте сервиса.
Наличие инструкции не заменяет проверяемые ограничения схемы и валидатора.

Пример:

```python
partition_col: IO.COLUMN_NAME | None = InputField(
    description="Column used to partition the read.",
    agent_description=(
        "Profile candidate key types, nulls, cardinality and skew before choosing. "
        "Use the exact raw catalog column name."
    ),
)
```
