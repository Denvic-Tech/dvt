...
class ExtensionStateManager:
    _local_locks_guard: ClassVar[threading.Lock] = threading.Lock()
    _local_locks: ClassVar[dict[tuple[str, str], threading.RLock]] = {}
    ...
    def _advisory_lock_key(extension_name: str, key: str) -> int:
        raw_key = f"{extension_name}:{key}".encode()
    ...
    def update_state(
        ...
        with (
            ...
            session.begin(),
            ExtensionStateManager._sync_lock_context(session, extension_name, key),
        ):
            extension = session.exec(
                ExtensionStateManager._state_select_stmt(
                    extension_name,
