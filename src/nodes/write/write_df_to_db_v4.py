...
class WriteDataFrameToDBV4SystemVariables(BaseModel):
...
class WriteDataFrameToDBV4(BaseNode):
    ...
    CATEGORY = "Writing"
    OUTPUT_NODE = True
    SYSTEM_VARIABLES_MODEL = WriteDataFrameToDBV4SystemVariables

    DESCRIPTION = (
        "Writes a DataFrame to an existing database table. This node does not create missing "
    ...
    def _build_request(self) -> WriteRequest:
        try:
            ...
            raise ValueError(f"Invalid write_v4 configuration: {exc}") from exc
    ...
    def _run_blocking_write_sync(self) -> tuple[WriteResult, Engine, bool]:
        ...
        try:
            ...
            logger.info(
                f"write_v4 completed: mode={result.mode} target={result.target_name} "
    ...
    async def process(self) -> None:
        ...
        try:
            ...
            self.emit_system_variables(
                WriteDataFrameToDBV4SystemVariables(
                    target_table=result.target_name,
                    rows_written=result.rows_written,
                )
            )
            logger.info("DataFrame successfully written to the database via write_v4.")
        finally:
