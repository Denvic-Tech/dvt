...
Обновлением управляет `extension_refresh.py`. Перед `shrink` контроллер ждёт
`WORKER_UP`: до этого ребёнок ещё может использовать унаследованный обработчик SIGTERM
родителя и не завершиться по сигналу. После выхода контроллер ждёт полного удаления ребёнка
из пула, асинхронно загружает registry, создаёт новый слот и возвращает подписки только после
его готовности. Celery event loop продолжает работать во время загрузки.
Выбор очередей также восстанавливается для последующих reconnect; QoS не меняется при
временном уменьшении пула, поскольку итоговая concurrency остаётся равной `1`.

Ожидание готовности ребёнка ограничено 30 секундами, загрузка registry — 60 секундами.
Если простаивающий ребёнок не вышел за 10 секунд после `shrink`, контроллер отправляет ему
SIGKILL и ждёт удаления ещё 5 секунд. Таймауты восстановления не прерывают активный запрос,
включая его result/ack обработку. Если после завершения задачи refresh нельзя безопасно
закончить, процесс выходит с кодом `70`; production Compose перезапускает контейнер по
`restart: unless-stopped`. Такой выход обходит graceful async cleanup, поскольку зависший
импорт ещё может занимать async runner или менять registry: создавать ребёнка из такого
состояния нельзя.
...
Свободный слот публикуется только при готовом соединении с брокером, обеих подписках,
готовом execution child и отсутствии refresh. Отсутствие активной задачи само по себе
не означает готовность воркера.

Read-only Celery inspect-команда `dvt_runtime_state` возвращает готовность и подписки,
этап refresh, категорию отказа, целевой размер пула, число запросов и PID/готовность детей.
Payload задач, конфигурация подключений и секреты в ответ не входят.

Регрессии `tests/integration/services/task_worker/test_extension_refresh_prefork.py`
запускают настоящий Linux prefork в изолированных процессах с memory transport:
два окна раннего SIGTERM и ребёнок, игнорирующий SIGTERM. Каждый сценарий проверяет
возврат подписок и выполнение следующего pool job. Внешние БД/брокер для них не нужны;
полный dispatch и загрузка расширений проверяются существующим Docker integration suite.
