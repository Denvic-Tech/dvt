# DVT Development Guide

Подробное руководство для разработчиков DVT: архитектура, локальное окружение, запуск сервисов,
Docker workflow, тестирование, расширение Node DSL и troubleshooting.
...
Built-in узел создаётся как отдельный package:

```text
src/nodes/<category>/<node_package>/
├── __init__.py
├── node.py
├── node.yaml
├── README.md       # optional, English
└── README.ru.md    # optional
```

1. В `node.yaml` укажите `schema_version: 1`.
2. Реализуйте ровно один регистрируемый класс узла в package и унаследуйтесь от подходящего базового класса (`DFOutputBaseNode`, `SqlConnectionOutputBaseNode` и т.д.).
3. В `__init__.py` экспортируйте публичный класс и назначьте его в `NODE_CLASS`.
4. Метаданные узла (`TITLE`, `CATEGORY`, `DESCRIPTION`, `TYPE`, `TAGS`, inputs/outputs) оставляйте в Node DSL/class, а не дублируйте в manifest.
5. Общий helper-код категории размещайте в `src/nodes/<category>/_shared/`; category `__init__.py` не должен импортировать узлы.
6. При необходимости colocate документацию в `README.md` и `README.ru.md`.
7. Обновите локализацию и проверьте регистрацию узла.
...
- [`../AGENTS.md`](../AGENTS.md) — инструкции для coding agents и описание сервисов
- `services/task_benchmarking/README.md` — benchmarking workflow
- `docs/` — дополнительная документация (эта директория)
