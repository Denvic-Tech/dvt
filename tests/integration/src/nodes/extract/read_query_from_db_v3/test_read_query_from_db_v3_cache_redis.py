...
async def test_read_query_from_db_v3_caches_partitions_and_preserves_types(
    resolved_sql_test_engine: sa.Engine,
    redis_container,
) -> None:
    engine = resolved_sql_test_engine
    family = dialect_family(engine)
    target_table = table_name("query_cache", engine)
    ...
    data_store = CodecObjectStore(
        RedisBlobStore(_store_settings(redis_container, f"it-read-query-cache/{uuid4().hex}")),
        data_codec,
    )
    data_index_store = RedisIndexStore(
        serializer=data_codec.dump,
        deserializer=data_codec.load,
        settings=_store_settings(redis_container, f"it-read-query-index/{uuid4().hex}"),
    )
